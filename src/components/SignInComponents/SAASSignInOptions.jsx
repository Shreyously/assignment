    ...
    <div className="py-5 px-3 flex flex-col items-center justify-center gap-2">
      {buttonItems.map((eachButtonItems, index) => (
        <button
          ...
          className="w-[100%] hover:bg-[#FAFAFA] disabled:bg-gray-50 disabled:cursor-not-allowed text-sm md:text-base md:w-[60%] py-3 font-semibold text-[#171717] rounded-lg border border-[#D8DAE5] flex flex-row items-center justify-center gap-2"
          onClick={() => handleSignIn(eachButtonItems.name)}
        >
          {loading === eachButtonItems.name ? (
            <LoadingSpinner text={`Signing in with ${eachButtonItems.name.split(' ').pop()}`} />
          ) : (
            <>
